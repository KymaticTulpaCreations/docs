...
KYMATIC and coins on Kymatic Chain are supported by the Ledger Nano S hardware wallet. The following describes how to use Ledger Nano S to connect to KYMATIC
...
!!! Tip
		The existing Ethereum App is compatible with Kymatic Chain
...
2) Within the "Manager" pane, type in "Ethereum" in the search field. Locate "Ethereum", then click on "Install". (Note: Do not use the "Kymatic Chain" App)
...
## Enabling contract data for KTC20 

!!! Tip
		 Please ensure your enabled contract data before transfering KTC20 tokens 
...
Kymatic Chain is compatible with MetaMask. Users can use MetaMask as a bridge between Ledger Nano S and Kymatic Chain to make transactions to the Kymatic Chain testnet.

1) Connect [MetaMask](./metamask.md) to KYMATIC Testnet by following this guide to add a custom network

 * Testnet
    * [RPC URLs](../developer/rpc.md)
    * ChainID: 0x5E0C (24076 in decimal)
    * Symbol: KYMATIC
    * [Block Explorer](https://testnet-explorer.kymaticscan.online)

 * Mainnet
    * [RPC URLs](../developer/rpc.md)
    * ChainID: 0x3C46 (15430 in decimal)
    * Symbol: KYMATIC
...
1. Fail to tranfer KTC20 tokens
