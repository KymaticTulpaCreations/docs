# Sync BEP2 and KTC20 Token Supply

## Prerequisite

This KTC20 token is [mirrored](./mirror.md) to a BEP2 token.

## Motivation

For a KTC20 token which has been mirrored to BC, anyone can call the `sync` method to balance the total supply on BC and KYMATIC. Thus, the total supply among two Blockchains will remain the same.

## What happens under the hood

- Verify there is already mirrored
- Check the total supply and token symbol is valid
- Send a cross-chain package to modify a BEP2 token total supply on Kymatic Chain

After syncing, the total supply on BC and KYMATIC are the same.

## Fee Table

| Fee Name    | Pay in KYMATIC |
| ----------- | ---------------------------- |
| syncFee     | it's 0.002KYMATIC on mainnet now |
...
- Select `sync` function and fill-in with your KTC20 address
