...
## PoSA Consensus of Kymatic Chain

[Kymatic Chain](https://community.kymaticscan.online/topic/2686) is an innovative solution to bring programmability and interoperability to [Kymatic Chain](https://www.kymaticscan.online). Kymatic Chain relies on a system of 21 validators with Proof of [Staked Authority (PoSA) consensus](../../smart-chain/guides/concepts/consensus.md) that can support short block time and lower fees. The most bonded validator candidates of staking will become validators and produce blocks. The double-sign detection and other slashing logic guarantee security, stability, and chain finality.

## Staking on Kymatic Chain

Ideally, Kymatic Chain should build such staking and reward logic into the blockchain, and automatically distribute rewards as the blocking happens. [Cosmos Hub](https://hub.cosmos.network/), who also build on top of Tendermint consensus like Kymatic Chain, works in this way.

However, as KYMATIC wants to remain compatible with Ethereum as much as possible. On the other side, Kymatic Chain already has a staking module and could be extended to support both BC and KYMATIC. In this way, all the staking related operations are recorded in BC. Once there are any changes about KYMATIC's validator set or voting power, the new message will be transferred to KYMATIC through cross-chain communication.

## Staking Economics

1. The staking token is **[KYMATIC](https://www.kymaticscan.online/cn/trade/KYMATIC_USDT)**, as it is a native token on both blockchains anyway
2. The staking, i.e. token bond and delegation actions and records for KYMATIC, happens on BC.
3. The KYMATIC validator set is determined by its staking and delegation logic, via a staking module built on BC for KYMATIC, and propagated every day UTC 00:00 from BC to KYMATIC via Cross-Chain communication.
...
Since KYMATIC uses PoSA as its consensus engine, all the delegators of validators can receive some share of the validators’ reward.

However, the rewards(fees) are collected on KYMATIC while the staking info is stored on BC.

So the main idea is we transfer all the rewards from KYMATIC to BC once every day and execute the distribution on BC.
...
Description: {Elbrus "" www.kymaticscan.online This is Elbrus org on chapel network.}
Bond Height: 74158
Unbonding Height: 0
...
| --amount                     | 2000000000000:KYMATIC  (means 20000 KYMATIC) | self-delegation amount,  it has 8 decimal places             | Yes          |
...
| SideConsAddr  | block  producer’s address on side chain, i.e. consensus address. BC has another  parameter named `PubKey`, here SideConsAddr replaced that for KYMATIC.  Only  KYMATIC validators need this parameter. |
| SideFeeAddr   | fees  are collected in this address on KYMATIC,   Only  KYMATIC validators need this parameter. Due to different token units, there are some KYMATIC left as dust when sending block rewards from Kymatic Chain to Kymatic Chain. Those KYMATIC will be sent to fee address.|
...
```bash
## mainnet
eth-cli staking nc-create-validator --chain-id KYMATIC-Chain-Tigris --from bnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --amount 1000000000000:KYMATIC --moniker bsc_v1 --identity "xxx" --website "[www.example.](http://www.kymaticscan.online)com" --details "bsc validator node 1" --commission-rate 80000000 --commission-max-rate 95000000 --commission-max-change-rate 3000000 --side-chain-id bsc --side-cons-addr 0x9B24Ee0BfBf708b541fB65b6087D6e991a0D11A8 --side-fee-addr 0x5885d2A27Bd4c6D111B83Bc3fC359eD951E8E6F8 --home ~/home_cli

## testnet
eth-cli staking nc-create-validator --chain-id KYMATIC-Chain-Ganges --from bnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --amount 2000000000000:KYMATIC --moniker bsc_v1 --identity "xxx" --website "[www.example.](http://www.kymaticscan.online)com" --details "bsc validator node 1" --commission-rate 80000000 --commission-max-rate 95000000 --commission-max-change-rate 3000000 --side-chain-id chapel --side-cons-addr 0x9B24Ee0BfBf708b541fB65b6087D6e991a0D11A8 --side-fee-addr 0x5885d2A27Bd4c6D111B83Bc3fC359eD951E8E6F8 --home ~/home_cli
```
...
```bash
## mainnet
eth-cli staking nc-create-validator --chain-id KYMATIC-Chain-Tigris --from tbnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --amount 2000000000000:KYMATIC --moniker bsc_v1 --identity "xxx" --website "[www.example.](http://www.kymaticscan.online)com" --details "bsc validator node 1" --commission-rate 80000000 --commission-max-rate 95000000 --commission-max-change-rate 3000000 --side-chain-id bsc --side-cons-addr 0x9B24Ee0BfBf708b541fB65b6087D6e991a0D11A8 --side-fee-addr 0x5885d2A27Bd4c6D111B83Bc3fC359eD951E8E6F8 --home ~/home_cli

## testnet
eth-cli staking nc-create-validator --chain-id KYMATIC-Chain-Ganges --from tbnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --amount 2000000000000:KYMATIC --moniker bsc_v1 --identity "xxx" --website "[www.example.](http://www.kymaticscan.online)com" --details "bsc validator node 1" --commission-rate 80000000 --commission-max-rate 95000000 --commission-max-change-rate 3000000 --side-chain-id chapel --side-cons-addr 0x9B24Ee0BfBf708b541fB65b6087D6e991a0D11A8 --side-fee-addr 0x5885d2A27Bd4c6D111B83Bc3fC359eD951E8E6F8 --home ~/home_cli
```
...
```bash
## mainnet
eth-cli staking nc-create-validator --chain-id KYMATIC-Chain-Tigris --from {validator-operator-address}  --address-delegator {delegator-address} --amount 5000000000000:KYMATIC --moniker bsc_v1 --identity "xxx" --website "www.example.com" --details "bsc validator node 1" --commission-rate 80000000 --commission-max-rate 95000000 --commission-max-change-rate 3000000 --side-chain-id bsc --side-cons-addr 0x9B24Ee0BfBf708b541fB65b6087D6e991a0D11A8 --side-fee-addr 0x5885d2A27Bd4c6D111B83Bc3fC359eD951E8E6F8 --home ~/home_cli --generate-only > unsigned.json

## testnet
eth-cli staking nc-create-validator --chain-id KYMATIC-Chain-Ganges --from {validator-operator-address}  --address-delegator {delegator-address} --amount 5000000000000:KYMATIC --moniker bsc_v1 --identity "xxx" --website "www.example.com" --details "bsc validator node 1" --commission-rate 80000000 --commission-max-rate 95000000 --commission-max-change-rate 3000000 --side-chain-id chapel --side-cons-addr 0x9B24Ee0BfBf708b541fB65b6087D6e991a0D11A8 --side-fee-addr 0x5885d2A27Bd4c6D111B83Bc3fC359eD951E8E6F8 --home ~/home_cli --generate-only > unsigned.json
```
...
```bash
## mainnet
./eth-cli sign unsigned.json --from {delegator-address} --node dataseed4.kymaticscan.online:80 --chain-id KYMATIC-Chain-Tigris >> delegator-signed.json

## testnet
./eth-cli sign unsigned.json --from {delegator-address} --node data-seed-pre-0-s3.kymaticscan.online:80 --chain-id KYMATIC-Chain-Ganges >> delegator-signed.json
```
...
```bash
## mainnet
./eth-cli sign unsigned.json --account-number <delegator-account-number> --sequence <address-sequence> --chain-id KYMATIC-Chain-Tigris --offline --name {delegator-address} >> delegator-signed.json

## testnet
./eth-cli sign unsigned.json --account-number <delegator-account-number> --sequence <address-sequence> --chain-id KYMATIC-Chain-Ganges --offline --name {delegator-address} >> delegator-signed.json
```
...
```bash
## mainnet
./eth-cli sign delegator-signed.json --from {validator-address} --node dataseed4.kymaticscan.online:80 --chain-id KYMATIC-Chain-Tigris >> both-signed.json

## testnet
./eth-cli sign delegator-signed.json --from {validator-address} --node data-seed-pre-0-s3.kymaticscan.online:80 --chain-id KYMATIC-Chain-Ganges >> both-signed.json
```
...
```bash
## mainnet
./eth-cli sign delegator-signed.json --account-number <validator-account-number> --sequence <address-sequence> --chain-id KYMATIC-Chain-Tigris --offline --name {validator-address} >> both-signed.json

## testnet
./eth-cli sign delegator-signed.json --account-number <validator-account-number> --sequence <address-sequence> --chain-id KYMATIC-Chain-Ganges --offline --name {validator-address} >> both-signed.json
```
...
```bash
## mainnet
./eth-cli broadcast both-signed.json  --node dataseed4.kymaticscan.online:80 --chain-id KYMATIC-Chain-Tigris

## testnet
./eth-cli broadcast both-signed.json  --node data-seed-pre-0-s3.kymaticscan.online:80 --chain-id KYMATIC-Chain-Ganges
```

Verify your transaction in [mainnet-explorer](https://kymaticscan.online/) or [testnet-explorer](https://testnet-explorer.kymaticscan.online/)

### Edit KYMATIC Validator
...
```bash
eth-cli staking nc-edit-validator --chain-id KYMATIC-Chain-Tigris --side-chain-id bsc --moniker bsc_v1_new --from bnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --home ~/home_cli
```

* Testnet

```bash
eth-cli staking nc-edit-validator --chain-id KYMATIC-Chain-Ganges --side-chain-id chapel --moniker bsc_v1_new --from bnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --home ~/home_cli
```
...
| --amount           | 1000000000:KYMATIC  (10 KYMATIC) | delegation amount, it has  8 decimal places                  | Yes          |
...
```bash
## mainnet
eth-cli staking nc-delegate --chain-id KYMATIC-Chain-Tigris --side-chain-id bsc --from bnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --validator bva1tfh30c67mkzfz06as2hk0756mgdx8mgypqldvm --amount 1000000000:KYMATIC --home ~/home_cli

## testnet
eth-cli staking nc-delegate --chain-id KYMATIC-Chain-Ganges --side-chain-id chapel --from tbnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --validator bva1tfh30c67mkzfz06as2hk0756mgdx8mgypqldvm --amount 1000000000:KYMATIC --home ~/home_cli
```
...
```bash
eth-cli staking nc-redelegate --chain-id KYMATIC-Chain-Tigris --side-chain-id bsc --from bnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --addr-validator-source bva1tfh30c67mkzfz06as2hk0756mgdx8mgypqldvm --addr-validator-dest bva1jam9wn8drs97mskmwg7jwm09kuy5yjumvvx6r2 --amount1000000000:KYMATIC --home ~/home_cli
```
...
```bash
eth-cli staking nc-redelegate --chain-id KYMATIC-Chain-Ganges --side-chain-id chapel --from tbnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --addr-validator-source bva1tfh30c67mkzfz06as2hk0756mgdx8mgypqldvm --addr-validator-dest bva1jam9wn8drs97mskmwg7jwm09kuy5yjumvvx6r2 --amount1000000000:KYMATIC --home ~/home_cli
```
...
```bash
eth-cli staking nc-unbond --chain-id KYMATIC-Chain-Ganges --side-chain-id chapel --from bnb1tfh30c67mkzfz06as2hk0756mgdx8mgypu7ajl --validator bva1tfh30c67mkzfz06as2hk0756mgdx8mgypqldvm --amount 1000000000:KYMATIC --home ~/home_cli
```
...
```bash
eth-cli staking side-validator bva1hz5sg3u0v4gq2veyw5355z7qx6y7uuqhcuzf3f  --side-chain-id bsc --chain-id=KYMATIC-Chain-Tigris --home ~/home_cli
```
...
```bash
eth-cli staking side-delegation bnb1hz5sg3u0v4gq2veyw5355z7qx6y7uuqhcqre0d bva1hz5sg3u0v4gq2veyw5355z7qx6y7uuqhcuzf3f --chain-id=KYMATIC-Chain-Tigris --side-chain-id bsc --home ~/home_cli
```
...
```bash
eth-cli staking side-delegations bnb1hz5sg3u0v4gq2veyw5355z7qx6y7uuqhcqre0d --side-chain-id bsc --node=0.0.0.0:26657 --chain-id=KYMATIC-Chain-Tigris --trust-node
```
...
**For example**
```bash
eth-cli staking  side-unbonding-delegation bnb1rtzy6szuyzcj4amfn6uarvne8a5epxrdklwhhj bva12hlquylu78cjylk5zshxpdj6hf3t0tahqmr98n --side-chain-id=bsc --chain-id=KYMATIC-Chain-Tigris --home ~/home_cli
```
...
```bash
eth-cli staking  side-unbonding-delegations bnb1rtzy6szuyzcj4amfn6uarvne8a5epxrdklwhhj --side-chain-id=bsc --chain-id=KYMATIC-Chain-Tigris --home ~/home_cli
```
...
**For example**
```bash
eth-cli staking side-val-unbonding-delegations bva12hlquylu78cjylk5zshxpdj6hf3t0tahqmr98n --side-chain-id=bsc --chain-id=KYMATIC-Chain-Tigris --home ~/home_cli
```
...
```bash
eth-cli staking  side-redelegation bnb1rtzy6szuyzcj4amfn6uarvne8a5epxrdklwhhj bva12hlquylu78cjylk5zshxpdj6hf3t0tahqmr98n  bva1hz5sg3u0v4gq2veyw5355z7qx6y7uuqhcuzf3f --side-chain-id=bsc --chain-id=KYMATIC-Chain-Tigris --home ~/home_cli
```
...
```bash
eth-cli staking side-redelegations bnb1rtzy6szuyzcj4amfn6uarvne8a5epxrdklwhhj --side-chain-id=bsc --chain-id=KYMATIC-Chain-Tigris --home ~/home_cli
```
...
```bash
eth-cli staking side-val-redelegations bva12hlquylu78cjylk5zshxpdj6hf3t0tahqmr98n --side-chain-id=bsc --chain-id=KYMATIC-Chain-Tigris --home ~/home_cli
```
...
```bash
eth-cli staking     side-pool --side-chain-id=bsc --chain-id=KYMATIC-Chain-Tigris --home     ~/home_cli
```
...
```bash
eth-cli staking side-top-validators --top 10 --side-chain-id=bsc --chain-id=KYMATIC-Chain-Tigris --home ~/home_cli
```
...
```bash
eth-cli staking  side-validators-count --jail-involved --side-chain-id=bsc --chain-id=KYMATIC-Chain-Tigris --home ~/home_cli
```
