...
[HKYMATIC](https://en.bitcoin.it/wiki/Hash_Time_Locked_Contracts) has been used for Atomic Swap and cross payment channel for a few years on Bitcoin and its variant blockchains, and also Ethereum. This BEP defines native transactions to support HKYMATIC on Kymatic Chain, and also proposes the standard infrastructure and procedure to use HKYMATIC for inter-chain atomic swap to easily create and use pegged token, which is called `Atomic Peg`.

## HKYMATIC on Kymatic Chain


As explained in [BEP3](https://github.com/githubusername/githubrepo/BEPs/blob/master/BEP3.md), Hash Timer Locked Contract(HKYMATIC) has been used for Atomic Swap and cross payment channels between different blockchains. BEP3 defines native transactions to support HKYMATIC on Kymatic Chain and also proposes the standard infrastructure and procedure to use HKYMATIC for inter-chain atomic swap to easily create and use pegged token.
...
```
AtomicSwapCoinsAccAddr = sdk.AccAddress(crypto.AddressHash([]byte("Kymatic ChainAtomicSwapCoins")))
```
...
Hash Timer Locked Transfer (HTLT) is a new transaction type on Kymatic Chain, to serve as HKYMATIC in the first step of Atomic Swap,
...
| ExpectedIncome | string | Expected income from swap counter party, example: "100:KYMATIC" or "100:KYMATIC,10000:BTCB-1DE" The amount needs to be bumped by e^8 | No |
...
Command line
```shell
./eth-cli token HTLT --recipient-addr <recipient-addr> --amount 100:KYMATIC --expected-income <expectedIncome> --height-span <heightSpan> --from <from-addr> --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```

Javascript
```javascript
  const client = new BncClient("https://testnet-dex.kymaticscan.online")
  ...
  const expectedIncome = "100:KYMATIC"// expected income
...
2. Swap from Kymatic Chain to Ethereum

* Clients send HTLT on Kymatic Chain on *testnet*:

Command line:
```shell
./eth-cli token HTLT --from <from-addr> --chain-id KYMATIC-Chain-Ganges  --height-span <heightSpan> --amount <amount> --expected-income <expectedIncome> --recipient-addr <deputy-bep2-addr>  --recipient-other-chain <client ethereum address>  --cross-chain --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```
...
| Amount | Coins | The swapped out amount BEP2 tokens, example: "100:KYMATIC" or "100:KYMATIC,10000:BTCB-1DE" | No |
...
Command line:
```shell
./eth-cli token deposit --swap-id <swapID>  --amount 10000:TEST-599 --from <from-key> --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```
...
Command line:
```shell
./eth-cli token claim --swap-id  <swapID> --random-number <random-number> --from <from-key> --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```
...
Command line:
```shell
./eth-cli token refund --swap-id <swapID> --from <from-key>  --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```
...
* On testnet:
```
./eth-cli token query-swap --swap-id <swapID> --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```
Expected output
```json
{
...
* On testnet:
```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr <address> --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```
...
* On testnet:
```
./eth-cli token query-swapIDs-by-creator  --creator-addr <address> --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```
...
-- | -- | -- | --
HTLT | N/A |  0.000375 KYMATIC | Y
depositHTLT | N/A |  0.000375 KYMATIC | Y
claimHTLT | N/A |  0.000375 KYMATIC | Y
refundHTLT | N/A |  0.000375 KYMATIC | Y
...
* Its corresponding address on testnet is: `tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr`on Kymatic Chain and `0xD93395B2771914E1679155F3EA58C41d89D96098` on Ethereum testnet

### Swap Tokens from Ethereum to Kymatic Chain
...
#### 3. Deputy Call HTLT on Kymatic Chain
Then, Deputy will send `HTLT` transaction [here](https://testnet-explorer.kymaticscan.online/tx/99CBC2896F0CF14DDAB0684BDA0A3E9FF2271056E68EC3559AB7FB24E0EE97DE)

#### 4. Claim HTLT on Kymatic Chain

* Get the `swapID` on Kymatic Chain

```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr tbnb1cs0j4p0p6d3fvd77zg3qzlwwgmrv3e9e63423w --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
[
...
* Send claim transaction on Kymatic Chain

```
./eth-cli token claim --swap-id  12aacc3bdc2cef97e8e45cc9b409796df57904a4e9c76863ad8420ff75f13128  --random-number <random-number> --from <from-key>  --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```

Example of `claim` tx on [testnet](https://testnet-dex.kymaticscan.online/api/v1/tx/6BA714E6D107F1D9634DDC159F560A1FB61393B8E15723EFD70B9EA8B0B1AA9A?format=json)
...
This is a javascript implementation for client app to swap [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) to [PPC-00A](https://testnet-explorer.kymaticscan.online/asset/PPC-00A) with deputy.
...
```javascript
  ...
  const deputyKYMATICWalletAddr = "tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr"
  ...
  const bnbClient = new BncClient("https://testnet-dex.kymaticscan.online")
  ...
  const bnbRPC = new rpcClient("https://seed-pre-s3.kymaticscan.online", "testnet")
  ...
  const hexEncodingClientKYMATICaddr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  ...
  const htltData = swapContract.methods.htlt("0x"+randomNumberHash, timestamp, heightSpan, deputyEthWalletAddr, "0x0", hexEncodingClientKYMATICaddr, amount, expectedIncome).encodeABI()
  ...
  //----------------------------------------------------------------------------
  //Step3 query swap created by deputy on Kymatic Chain and verify swap parameters
  //----------------------------------------------------------------------------
  const swapID = calculateSwapID(randomNumberHash.replace("0x", ""), deputyKYMATICWalletAddr, clientEthWalletAddr).toString()
  console.log(swapID)
...
### Swap Tokens from Kymatic Chain to Ethereum
![image-20190918193910521](../../assets/bnc2eth.png)

#### 1. Send `HTLT` Transaction from Kymatic Chain
...
```
./eth-cli token HTLT --from atomic --recipient-addr tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr  --chain-id KYMATIC-Chain-Ganges  --height-span 10000 --amount  9900000000:PPC-00A  --expected-income 9900000000:PPC  --recipient-other-chain 0x133D144F52705cEb3f5801B63b9EBcCF4102f5Ed  --cross-chain --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
...
Example is [here](https://testnet-explorer.kymaticscan.online/tx/9ECECE9E0F08EE78583CFA37FD4C3F03521289F0F229A612886B8B21B9C62D7F)

Then, you can query the the swap by `SwapID`:

```
./eth-cli token query-swap --swap-id f85dd907df0a5897927b949c0f9e2563d453ba698ff9941fed1ce91f8057afc2 --chain-id KYMATIC-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.kymaticscan.online:80
```
...
- `_randomNumberHash` must equal to the randomNumberHash in client HTLT transaction on Kymatic Chain
- `_recipientAddr` must equal to client ethereum wallet address
- `_timestamp` must equal to the timestamp in client HTLT transaction on Kymatic Chain
...
#### 5. Deputy Claim on Kymatic Chain

`Claim HTLT` transaction from **Deputy** is [sent](https://testnet-explorer.kymaticscan.online/tx/8C616DEFD2EAA41E13D2DC4844B218DFF8CFE24B4C7A693AAD700381B5FF7B48) afterwards:

#### 6. Demo for Client APP: swap bep2 to erc20

This is a javascript implementation for client app to swap  [PPC-00A](https://testnet-explorer.kymaticscan.online/asset/PPC-00A) to [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) with deputy.
...
```javascript
  ...
  //--------------------------------------------
  //Step1 send htlt on Kymatic Chain
  //--------------------------------------------
  const randomNumber = "e8eae926261ab77d018202434791a335249b470246a7b02e28c3b2fb6ffad8f3"
  const timestamp = Math.floor(Date.now()/1000)
  const randomNumberHash = calculateRandomNumberHash(randomNumber, timestamp).toString("hex")
  ...
  bnbClient.swap.HTLT(clientBnbWalletAddr, deputyKYMATICWalletAddr, clientEthWalletAddr, "", randomNumberHash, timestamp, amount, expectedIncome, heightSpan, true)
  await wait(1000)
  ...
  //----------------------------------------------------------------------------
  const hexEncodingClientKYMATICaddr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  const swapID = await swapContract.methods.calSwapID("0x"+randomNumberHash, deputyEthWalletAddr, hexEncodingClientKYMATICaddr).call()
  console.log(swapID)
  ...
  //----------------------------------------------------------------------------
  //If step2 or step3 are failed and the expire height on Kymatic Chain is passed, try to send refundHTLT transaction on Kymatic Chain
  //----------------------------------------------------------------------------
```
