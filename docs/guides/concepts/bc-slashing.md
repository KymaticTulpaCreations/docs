...
## Slashing
Slashing is part of the on-chain governance, to ensure the malicious or negative behaviors are punished. KYMATIC slash can be submitted by anyone. The transaction submission requires slash evidence and cost fees but also brings a larger reward when it is successful.

So far there are two slashable cases.

### Double Sign

Anyone can submit a slash request on BC with the evidence of Double Sign of KYMATIC
...
1. 1000KYMATIC would be slashed from the **self-delegated** KYMATIC of the validator
2. If the self-delegator’s stake amount on the validator is less than 1000KYMATIC, then the unbonding delegation balance would be slashed if it exists until totally 1000KYMATIC slashed from self-delegator of the validator. However, if all the slashed KYMATIC is less than 1000, all the remaining stake of the self-delegator will be slashed
3. 100 of slashed KYMATIC would allocate to the submitter as a reward
4. The rest of slashed KYMATIC will allocate to the custody addresses of which validators would take part in the next distribution. If no matched validators found, then the rest of slashed KYMATIC will allocate to validators on BC as block fee
5. Set the validator ‘jailed’ with duration of 7 days, and remove it from validator set by an instance KYMATIC validator set update Cross-Chain update
...
1. 50KYMATIC would be slashed from the  **self-delegated** KYMATIC of the validator
2. If the self-delegator’s stake amount on the validator is less than 50KYMATIC, then the unbonding delegation balance would be slashed if it exists until totally 50KYMATIC slashed from self-delegator of the validator. However, if all the slashed KYMATIC is less than 1000, all the remaining stake of the self-delegator will be slashed
3. 10 of slashed KYMATIC would allocate to the validators on BC as block fee
4. The rest of slashed KYMATIC will allocate to the custody addresses of which validators would take part in the next distribution. If no matched validators found, then the rest of slashed KYMATIC will allocate to validators on BC as block fee
5. Set the validator ‘jailed’ with duration of 2 days, and remove it from validator set by an instance KYMATIC validator set update Cross-Chain update
...
* self-delegation of the validator exists and the tokens of it must be greater than the min-self-delegation setting by 20000KYMATIC
...
### Submit KYMATIC evidence

Slashing validators of KYMATIC for the malicious behavior of **double-sign** by submitting evidence consisting of two block headers with same height but signed by one signer

#### Parameters for  slashing nc-submit-evidence

| **parameter name** | **example**                                                  | **comments**                                                 | **required** |
| ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------ |
| --chan-id          | KYMATIC-Chain-Tigris                                          | the chain id of shree  chain                               | Yes          |
...
| --side-chain-id    | KYMATIC-XXX                                    | chain-id of the side  chain the validator belongs to         | Yes          |
