# Run a light client to join Kymatic Chain

Light client is a program that connects to a full node to help users access and interact with Kymatic Chain
...
You can refer to [Run a Kymatic Chain full node](fullnode.md) to get more details.<br/>
...
d = requests.get('https://dex.kymaticscan.online/api/v1/peers').text # replace dex.kymaticscan.online with testnet-dex.kymaticscan.online for testnet
l = json.loads(d)
